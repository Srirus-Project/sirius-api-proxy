...
impl<S: Subscriber> Layer<S> for AppLayer {
    ...
    fn on_event(&self, event: &Event<'_>, _: Context<'_, S>) {
        ...
        let Ok(fields) = serde_json::to_string(&record.fields) else {
            return;
        };
        let line = match self.config.format {
            Format::Json => match serde_json::to_string(&record) {
                Ok(line) => line,
                Err(_) => return,
            },
            Format::Text => format!(
...
const APP_TARGET: &str = "sirius_api_proxy";
const APP_PREFIX: &str = "sirius_api_proxy::";
