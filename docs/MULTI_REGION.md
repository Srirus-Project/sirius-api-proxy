...
The top-level `listen` binds one HTTP listener. `regions` maps region names to the
same client settings used by a single-region file, excluding `listen`. Each map
